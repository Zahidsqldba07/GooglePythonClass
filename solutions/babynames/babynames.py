...
Suggested milestones for incremental development:
 ...
 -Get the names log_data into a dict and print it
...
def extract_names(filename):
    """
    Given a file name for baby.html, returns a list starting with the year string followed by the name-rank strings in
    alphabetical order.  ['2006', 'Aaliyah 91', Aaron 57', 'Abagail 895', ' ...]
    """
    # +++your code here+++
    # LAB(begin solution)
    ...
    # Extract all the log_data tuples with a findall()
    # each tuple is: (rank, boy-name, girl-name)
    tuples = re.findall(r'<td>(\d+)</td><td>(\w+)</td>\<td>(\w+)</td>', text)
    # print tuples

    # Store log_data into a dict using each name as a key and that name's rank number as the value.
...
def main():
    ...
    # +++your code here+++
    # For each filename, get the names, then either print the text output or write it to a summary file
    # LAB(begin solution)
